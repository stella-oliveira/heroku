<?php
    ...
    function login() {
        ...
        if (!empty($_SESSION)) {
            ...
            if (isset($_POST['logar_cpf'])) {    
                $sql = "SELECT usuario FROM pessoa_fisica WHERE usuario ='" . $_SESSION['usuario'] . "' AND senha=SHA1('" . $_SESSION['senha'] . "')";
                ...
                if (!$num_rows) {
                    $_SESSION['erro'] = "Usuário ou senha inválido CPF!";
                    header("Location: login.php");
                    exit();
                }
            } else if (isset($_POST['logar_cnpj'])) {
                $sql = "SELECT usuario FROM pessoa_juridica WHERE usuario ='" . $_SESSION['usuario'] . "' AND senha=SHA1('" . $_SESSION['senha'] . "')";
